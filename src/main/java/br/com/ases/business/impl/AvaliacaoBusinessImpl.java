...
public class AvaliacaoBusinessImpl implements AvaliacaoBusiness {
	...
	public Nota obterNotaEselo(Source documento, String url, List<Entry<OccurrenceKey, List<Occurrence>>> mapaListaOcorrencias) {
		...
		String recomendacaoAntiga = ""; // Serve identificar a mudan�a de recomenda��o										
		String recomendacao = "";
		...
		try {
			...
			while ((linha = reader.readLine()) != null) {
				...
				//Separa a recomenda��o do crit�rio sendo (x.y) de (x.y.z)
				recomendacaoProperties = recomendacaoCriterio[0].toString() + "." + recomendacaoCriterio[1].toString();
				
				//Semara a crit�rio da recomenda��o sendo (z) de (x.y.z)		
				...
				// Fim adiciona a recomenda��o da lista do properties
	...
	public Nota obterNota(List<SummarizedOccurrence> occurrences, String url,
		...
		try {
			...
			/*
			 * for(Integer rn : rns){ //Recomenda��o Avaliada //
			 * postParams.addParam
		...
		/*
		 * Nota nota = null ; //Vers�o desenvolvida por Rodrigo try {
	...
	// Passa a recomenda��o do Ases como chave e retorna a recomenda��o do Eselo
